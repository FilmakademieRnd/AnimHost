...
QJsonObject GNNNode::save() const
{
...
void GNNNode::load(QJsonObject const& p)
{
...
NodeDataType GNNNode::dataPortType(QtNodes::PortType portType, QtNodes::PortIndex portIndex) const
{
    NodeDataType type;
    if (portType == QtNodes::PortType::In)
        switch (portIndex) {
...
void GNNNode::processInData(std::shared_ptr<NodeData> data, QtNodes::PortIndex portIndex)
{
    if (!data) {
...
std::shared_ptr<NodeData> GNNNode::processOutData(QtNodes::PortIndex port)
{
    switch (port) {
    case 0:
...
QWidget* GNNNode::embeddedWidget()
{
    if (!_widget) {
       _widget = new QWidget();
       ...
       _mixControlPathTranslation->setValue(0.5);
       ...
       _mixControlPathRotation->setValue(0.3);
...
void GNNNode::onFileSelectionChanged()
{
